...
import { 
  ...
  ChevronRight, Home, Menu, X, Sun, Moon, Upload, BookOpen
} from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-white/80 dark:bg-gray-900/80 border-b border-gray-200 dark:border-gray-800 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 shrink-0">
            <div className="bg-blue-600 p-2 rounded-lg cursor-pointer" onClick={() => setView('tool')}>
              <FileOutput className="w-5 h-5 text-white" />
            </div>
            <h1 
              className="text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent cursor-pointer hidden md:block"
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {status === ProcessStatus.SUCCESS && view === 'tool' && (
               <button 
                 onClick={resetTool}
                 className="hidden sm:flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-lg hover:shadow-blue-500/30 transition-all transform hover:-translate-y-0.5"
               >
            ...
            {/* Mobile simplified upload button */}
             {status === ProcessStatus.SUCCESS && view === 'tool' && (
               <button 
                 onClick={resetTool}
                 className="sm:hidden flex items-center justify-center p-2 text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-lg"
               >
